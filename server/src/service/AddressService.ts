...
export class AddressServiceImpl implements AddressService {
  ...
  public async getAddresses(): Promise<Array<Address>> {
    ...
    const addressesMongo: Array<
      Address
    > = await this.addressRepositoryMongo.findAll().then(a =>
      a.map((dto: AddressDTO) => {
  ...
  public async createAddress(address: Address): Promise<Address> {
    ...
    const createdDTO: AddressDTO = await this.addressRepositoryMongo.create(
      addressDTO
    );
  ...
  public async updateAddress(address: Address): Promise<Address> {
    ...
    const updated: AddressDTO = await this.addressRepositoryMongo.update(
      addressDTO
    );
