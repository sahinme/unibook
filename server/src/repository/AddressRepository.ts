...
export class AddressRepositoryImplMongo implements AddressRepository {
  public async findAll(): Promise<Array<AddressDTO>> {
    const addressDTOs = await mongoDatabase
      .connect()
      .then(() => mongoDatabase.Addresses.find());
  ...
  public async create(addressDTO: AddressDTO): Promise<AddressDTO> {
    return await mongoDatabase
      .connect()
      .then(() => mongoDatabase.Addresses.create(addressDTO));
  }

  public async update(addressDTO: AddressDTO): Promise<AddressDTO> {
    const dto: AddressMongoSchema = await mongoDatabase
      .connect()
      .then(() => mongoDatabase.Addresses.findOne(addressDTO._id));
  ...
  public async find(id: string): Promise<AddressDTO> {
    return await mongoDatabase
      .connect()
      .then(() => mongoDatabase.Addresses.findOne(id));
  }
}
